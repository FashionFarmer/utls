...
type Conn struct {
	...
	// visionReadDetached and visionWriteDetached guard the one-way transition
	// from the TLS record layer to the raw connection used by XTLS Vision.
	// They are protected by in and out respectively.
	visionReadDetached  bool
	visionWriteDetached bool
...
// DetachVisionRead atomically hands the read side of c to an XTLS Vision
// direct-copy codec. buffered contains, in order, plaintext already decoded
// from the current TLS record followed by bytes read ahead from the raw
// connection. The caller must consume buffered before reading raw.
//
// Detaching is irreversible. No goroutine may call Read concurrently with
// this method, and callers must not use Read after it succeeds.
func (c *Conn) DetachVisionRead() (raw net.Conn, buffered []byte, err error) {
	c.in.Lock()
	defer c.in.Unlock()
	if c.visionReadDetached {
		return nil, nil, errors.New("tls: Vision read side already detached")
	}
	buffered = make([]byte, 0, c.input.Len()+c.rawInput.Len())
	if c.input.Len() > 0 {
		plaintext := make([]byte, c.input.Len())
		_, _ = io.ReadFull(&c.input, plaintext)
		buffered = append(buffered, plaintext...)
	}
	buffered = append(buffered, c.rawInput.Bytes()...)
	c.input.Reset(nil)
	c.rawInput.Reset()
	c.visionReadDetached = true
	return c.conn, buffered, nil
}

// DetachVisionWrite waits for any active TLS write and then hands the write
// side of c to an XTLS Vision direct-copy codec. Detaching is irreversible;
// callers must not use Write after it succeeds.
func (c *Conn) DetachVisionWrite() (net.Conn, error) {
	c.out.Lock()
	defer c.out.Unlock()
	if c.visionWriteDetached {
		return nil, errors.New("tls: Vision write side already detached")
	}
	c.visionWriteDetached = true
	return c.conn, nil
}
...
func (c *Conn) Write(b []byte) (int, error) {
	...
	defer c.out.Unlock()
	if c.visionWriteDetached {
		return 0, errors.New("tls: Write called after Vision write detach")
	}
...
func (c *Conn) Read(b []byte) (int, error) {
	...
	defer c.in.Unlock()
	if c.visionReadDetached {
		return 0, errors.New("tls: Read called after Vision read detach")
	}
